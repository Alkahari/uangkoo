...
class $CategoriesTable extends Categories
  ...
  late final GeneratedColumn<int> id = GeneratedColumn<int>(
    ...
    defaultConstraints: (p0) => 'PRIMARY KEY AUTOINCREMENT',
  );
...
abstract class _$AppDb extends GeneratedDatabase {
  ...
  Future<int> deleteCategory(int categoryId) async {
    return await (delete(categories)..where((tbl) => tbl.id.equals(categoryId)))
        .go();
  }

  Future<int> deleteTransaction(int transactionId) async {
    return await (delete(transactions)
          ..where((tbl) => tbl.id.equals(transactionId)))
        .go();
  }

  Future<int> updateCategory(Category updatedCategory) async {
    return await (update(categories)
          ..where((tbl) => tbl.id.equals(updatedCategory.id)))
        .write(updatedCategory);
  }

  Future<int> updateTransaction(Transaction updatedTransaction) async {
    return await (update(transactions)
          ..where((tbl) => tbl.id.equals(updatedTransaction.id)))
